...
class Attendance:
    ...
    def _dates_of_holidays_and_non_holidays(self, dates, legal_holidays) -> Tuple[List[WorkingDate], List[WorkingDate]]:
        dates_of_holidays = [date for date in dates.values() if date.is_legal_holiday(legal_holidays)]
        dates_of_non_holidays = [date for date in dates.values() if not date.is_legal_holiday(legal_holidays)]
    ...
    def weekly_overtime_work_hours(self, working_hours_per_day, legal_holidays) -> Dict[int, Dict[int, timedelta]]:
        overtime_hours = {}
        for (year, week_number), week in self._isocalendar_based_dates.items():
            dates_of_holidays, dates_of_non_holidays = self._dates_of_holidays_and_non_holidays(week, legal_holidays)
    ...
    def monthly_overtime_work_hours(self, working_hours_per_day, legal_holidays) -> Dict[int, Dict[int, timedelta]]:
        overtime_hours = {}
        for (year, month_number), month in self._monthly_based_dates.items():
            dates_of_holidays, dates_of_non_holidays = self._dates_of_holidays_and_non_holidays(month, legal_holidays)
...
class Agreement36:
    EXPIRATION_DATE = timedelta(days=364)
    ...
    def __init__(self, starting_date: datetime,
        ...
        self._ending_date = starting_date + self.EXPIRATION_DATE
...
class Validator:
    ...
    def validate(self, attendance: Attendance) -> Result:
        ...
        if not self._profile.has_agreement36():
            ...
            if violated:
                violations += 'Must be no overtime'
        else:
            ...
            if violated:
                violations += 'Montly overtime must be 45 hours or less'
            ...
            if violated:
                violations += 'Yearly overtime must be 360 hours or less'
